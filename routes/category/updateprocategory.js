...
var jwtkey = "shopping"
/*
	接口：updateprocategory
	方法：post
	参数：product_like_id，product_like_name，product_like_status
*/ 

router.post("/updateprocategory", function (req, res) {
	...
	jwt.verify(token, jwtkey, function (err, data) {
		if(err) {
			...
			} else if (product_like_name == undefined) {
				res.json({code: "1", msg: "类别名称为空", data: ""})
			} else if (product_like_status == undefined) {
				res.json({code: "1", msg: "状态为空", data: ""})
			} else {
				// 编辑分类
				var sql = "UPDATE product_like SET product_like_name=?,product_like_status=?,update_time=? WHERE product_like_id = ?"
				var sqlparams = [product_like_name, product_like_status, date, product_like_id]
